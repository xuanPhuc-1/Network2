...
{
  uint32_t stream = 2;                                                 //dif
  std::string socketFactory = "ns3::TcpSocketFactory";
  std::string tcpTypeId = "ns3::TcpLinuxReno";                        //set up type algo
  ...
  routers.Create (2);         //routers[0,1]
  leftNodes.Create (2);       //leftNodes[0,1]
  rightNodes.Create (2);      //rightNodes[0,1]

  std::vector <NetDeviceContainer> leftToRouter;
  std::vector <NetDeviceContainer> routerToRight;
// Our Network topology
//            leaf1          p-prouter      leaf1
//       ln0 ---------- r0 ---------- r1 ---------- rn0
//                      |             |
//                      |leaf2        |leaf2
//                      |             |
...
//
  ...
  routerToRight.push_back(pointToPointLeaf2.Install(routers.Get(1), rightNodes.Get(1)));
  ...
  InstallBulkSend (leftNodes.Get (1), routerToRightIPAddress [1].GetAddress (1), port, socketFactory, 2, 0, MakeCallback (&CwndChange));
  InstallBulkSend (leftNodes.Get (0), routerToRightIPAddress [1].GetAddress (1), port, socketFactory, 2, 0, MakeCallback (&CwndChange));
